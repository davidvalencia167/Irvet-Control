...
function PetCard({ pet, index, catalogo, onChange, onRemove, canRemove }: {
  pet: PetEntry; index: number; catalogo: Record<string, ServicioCatalogo[]>;
  onChange: (id: string, patch: Partial<PetEntry>) => void; onRemove: (id: string) => void; canRemove: boolean;
}) {
  ...
  return <div className="border border-[rgba(27,43,75,0.1)] rounded-2xl overflow-hidden">
    ...
    {open && <div className="p-4 space-y-5 border-t border-[rgba(27,43,75,0.06)]">
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="col-span-2"><Label text="Correo Electrónico"/><input value={pet.correo} onChange={e => update({ correo: e.target.value })} placeholder="correo@email.com" className={IC}/></div>
...
function PaymentBlock({ pagos, valorTotal, onAddRow, onUpdateRow, onRemoveRow }: { pagos: PaymentRow[]; valorTotal: string; onAddRow: () => void; onUpdateRow: (id: string, key: keyof PaymentRow, val: string) => void; onRemoveRow: (id: string) => void; }) {
  const total = Number(valorTotal || 0);
  const totalPagado = pagos.reduce((a,p) => a + Number(p.valor || 0), 0);
  const pendiente = Math.max(0, total - totalPagado);
  const estado = total > 0 && totalPagado >= total ? "Pagado" : totalPagado > 0 ? "Pago parcial" : "Pendiente por pago";
  return <div className="space-y-5">
    <div className="grid grid-cols-3 gap-4"><div className="bg-[#F8FAFC] rounded-xl p-3"><p className="text-[10px] font-bold text-[#6B7A99] uppercase">Valor total</p><p className="text-[18px] font-extrabold text-[#1B2B4B]">${total.toLocaleString("es-CO")}</p></div><div className="bg-[#F8FAFC] rounded-xl p-3"><p className="text-[10px] font-bold text-[#6B7A99] uppercase">Total abonado</p><p className="text-[18px] font-extrabold text-emerald-600">${totalPagado.toLocaleString("es-CO")}</p></div><div className={`${pendiente > 0 ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200"} rounded-xl p-3 border`}><p className="text-[10px] font-bold text-[#6B7A99] uppercase">Saldo</p><p className={`text-[18px] font-extrabold ${pendiente > 0 ? "text-amber-700" : "text-emerald-700"}`}>${pendiente.toLocaleString("es-CO")}</p><span className="text-[10px] font-bold">{estado}</span></div></div>
    <div><div className="flex items-center justify-between mb-3"><Label text="Métodos de pago"/><button type="button" onClick={onAddRow} className="flex items-center gap-1.5 text-[12px] font-bold text-[#2BB5C3]"><Plus size={14}/>Agregar método</button></div>{pagos.length === 0 && <div className="text-[12px] text-[#6B7A99] bg-[#F8FAFC] rounded-xl p-4">No se ha registrado ningún pago. La orden puede quedar pendiente de pago.</div>}<div className="space-y-2">{pagos.map((row,i) => <div key={row.id} className="flex gap-3 items-center"><div className="w-6 h-6 rounded-full bg-[#F4F7FA] flex items-center justify-center text-[10px] font-bold text-[#6B7A99]">{i+1}</div><select value={row.medio} onChange={e => onUpdateRow(row.id,"medio",e.target.value)} className={`${IC} flex-1`}><option value="">Medio de pago...</option>{MEDIOS_PAGO.map(m => <option key={m}>{m}</option>)}</select><input type="number" min="0" value={row.valor} onChange={e => onUpdateRow(row.id,"valor",e.target.value)} placeholder="$0" className={`${IC} w-36`}/><button type="button" onClick={() => onRemoveRow(row.id)} className="p-1.5 rounded-lg bg-rose-50 text-rose-400"><Trash2 size={13}/></button></div>)}</div></div>
  </div>;
}
...
export default function OrderForm({ factura, editingId, initialForm, errors, catalogo, onSave, onClear, onCancelEdit }: OrderFormProps) {
  ...
  const addPay = () => setForm(f => ({...f, pagos:[...f.pagos,{id:crypto.randomUUID(),medio:"",valor:""}]}));
